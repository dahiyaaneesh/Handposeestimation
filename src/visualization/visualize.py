...
def draw_hand_over_image(frame: np.array, predicted_joints: np.array) -> np.array:
    """
    A hand stick figure will be drawn over the frame using the predicted joints. This is the sister function of
    plot_hand for opencv frame.

    Args:
        frame (np.array)
        predicted_joints (np.array)
    ...
    colors = (
        np.array(
            read_json(
                os.path.join(
                    MASTER_THESIS_DIR, "src", "visualization", "joint_color.json"
                )
            )["joint_color2"]
        )
        * 255
    )
    freihand_mapped_joints = joints.ait_to_freihand(predicted_joints)
    bones = [
        ((i, i + 1), colors[1 + i, :]) if i % 4 != 0 else ((0, i + 1), colors[1 + i, :])
        for i in range(0, 20)
    ]
    for connection, color in bones:
        coord1 = freihand_mapped_joints[connection[0], :]
        coord2 = freihand_mapped_joints[connection[1], :]
        coords = np.stack([coord1, coord2])
        color = [int(c) for c in color]
        frame = cv2.line(
            frame, (coords[0, 0], coords[0, 1]), (coords[1, 0], coords[1, 1]), color, 2
        )
    # Drawing joints
    for i in range(21):
        ...
        frame = cv2.circle(
            frame,
            (int(freihand_mapped_joints[i, 0]), int(freihand_mapped_joints[i, 1])),
            2,
            color,
            5,
        )
